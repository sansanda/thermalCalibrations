...
public class CalibrationSetUp_DevicesToCalibrate_JPanel extends JPanel {
	...
	private static final int 	N_DEVICES_TO_CALIBRATE = 10;
	private static final int 	N_MULTIMETER_CHANNELS = 22;
				...
				checkBox.addItemListener(new ItemListener()
			    {
			        public void itemStateChanged(ItemEvent e)
			        {
			        	...
			        	if (e.getStateChange()==ItemEvent.SELECTED){
			        		...
			        		if (getDeviceCheckBoxNumber(checkBoxNumber+1)!=null) getDeviceCheckBoxNumber(checkBoxNumber+1).setEnabled(true);
			        	}
	...
	private void initializeDevicesMultimeterChannelsComboBoxes() {
		...
		for (int i=1;i<=N_DEVICES_TO_CALIBRATE;i++){
			...
			for (int j=MIN_MULTIMETER_CHANNEL_NUMBER;j<=N_MULTIMETER_CHANNELS;j++){
				if (!(temperatureSensor.getChannel()==j) & !((temperatureSensor.getChannel()+10)==j)) comboBox.addItem(j);
			}
			ItemListener deviceItemListener  = new ItemListener()
		    {
		        public void itemStateChanged(ItemEvent e)
		        {
		        	...
		        	if (temperatureSensor.getChannel()==deviceMultimeterChannel || (temperatureSensor.getChannel()+10)==deviceMultimeterChannel){
		           	  JOptionPane.showMessageDialog(null,"WARNING: TEMPERATURE SENSOR CHANNEL = " + temperatureSensor.getChannel());
		         	  JOptionPane.showMessageDialog(null,"DEVICE CHANNEL MUST BE DIFFERENT FROM CHANNEL " + temperatureSensor.getChannel());

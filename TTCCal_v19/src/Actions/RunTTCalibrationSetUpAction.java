...
public class RunTTCalibrationSetUpAction implements Action{
		...
		worker = new SwingWorker() {
			public Object construct() {
				try {
					...
					switch (program.getTemperatureStabilizationCriteriumData().getTemperatureStabilitzationMethod()){
					...
					case 1:		//Temperature Stabilitzation by Time
						...
						{
							waitForTemperatureStabilizationTime(program,temperatureSensorData);
							//En este momento la temperatura es estable, guardamos los datos
							//de consigna, Tª en Pt100, Resistencia en Pt100 en un fichero txt
							pt100RealT = read2700ChannelsAndSaveDataToFile(program,temperatureSensorData);
						}
	...
	private boolean isActualOvenTempBelowActualProgramStep(float _pt100RealT,TTCSetUpData _program,int _actualProgramStep){
	...
	private int setTheActualProgramStepAboveTheRealTemp(float _pt100RealT,TTCSetUpData _program)throws Throwable{
	...
	private void waitForTemperatureStabilizationTime(TTCSetUpData _program,TemperatureSensorData _temperatureSensorData) throws Exception{
		printActionMessageAndProgressScreenMessage("Waiting... "+_program.getTemperatureStabilizationCriteriumData().getTemperatureStabilitzationTime()+" minutes for Temperature Stabilization ");
		long actualTime = System.currentTimeMillis();
		long futureTime = System.currentTimeMillis() + _program.getTemperatureStabilizationCriteriumData().getTemperatureStabilitzationTime()*60*1000;
	...
	private void waitForTemperatureStandardDeviationMatch(TTCSetUpData _program,TemperatureSensorData _temperatureSensorData) throws Exception{
		float 	pt100RealT = 0;
		float 	actualStepStandardDeviation = 0;
		int 	actualNPassesBelowStandardDeviation = 0;
		printActionMessageAndProgressScreenMessage("Taking "+_program.getTemperatureStabilizationCriteriumData().getMeasurementWindow()+" temperature measures in "+(_program.getTemperatureStabilizationCriteriumData().getSamplingPeriode()*_program.getTemperatureStabilizationCriteriumData().getMeasurementWindow())/1000+" seconds and calulating the Standard Deviation\n");
		actualStepStandardDeviation = K2700.takeNTemperatureMeasuresWithDelayAndReturnStDev(_temperatureSensorData.getChannel(),_program.getTemperatureStabilizationCriteriumData().getMeasurementWindow(),_program.getTemperatureStabilizationCriteriumData().getSamplingPeriode());
		if (actualStepStandardDeviation<=_program.getTemperatureStabilizationCriteriumData().getStDev()) actualNPassesBelowStandardDeviation++;
		...
		pt100RealT = K2700.measurePT100Temperature(_temperatureSensorData.getChannel());
		insertTempPointsInGraph(System.currentTimeMillis(), pt100RealT,program.getTemperatureProfileData().getTemperatures()[actualProgramStep]);
		while (actualNPassesBelowStandardDeviation<_program.getTemperatureStabilizationCriteriumData().getNumberOfWindowsUnderStDev()){
			printActionMessageAndProgressScreenMessage("Standard Deviation is over than "+_program.getTemperatureStabilizationCriteriumData().getStDev()+". Continue reading temperatures.\n");
			actualStepStandardDeviation = K2700.takeNTemperatureMeasuresWithDelayAndReturnStDev(_temperatureSensorData.getChannel(),_program.getTemperatureStabilizationCriteriumData().getMeasurementWindow(),_program.getTemperatureStabilizationCriteriumData().getSamplingPeriode());
			if (actualStepStandardDeviation<=_program.getTemperatureStabilizationCriteriumData().getStDev()) actualNPassesBelowStandardDeviation++;
			else actualNPassesBelowStandardDeviation = 0;
			printActionMessageAndProgressScreenMessage("Standard Deviation = "+actualStepStandardDeviation);
		...
		printActionMessageAndProgressScreenMessage("Standard Deviation is less than "+_program.getTemperatureStabilizationCriteriumData().getStDev()+" during "+_program.getTemperatureStabilizationCriteriumData().getNumberOfWindowsUnderStDev()+" consecutive passes."+"\n");
	}
	...
	private float read2700ChannelsAndSaveDataToFile(TTCSetUpData _program,TemperatureSensorData _temperatureSensorData) throws Exception{
		...
		for (int i=0;i<_program.getTTCsToCalibrateData().getNDevicesToMeasure();i++){
			...
			channelsToMeasure[i] = K2700.measureAverage4WireResistance(program.getTTCsToCalibrateData().getDevicesToMeasureData()[i].getDeviceChannel(),avg);
		}
	...
	private int initializeProgram()throws Exception{
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
    		...
    		program = new TTCSetUpData(programFilePath);
    		if (program.getTTCsToCalibrateData().getNDevicesToMeasure()==0){
	...
	private int initializeResultsFile(TTCSetUpData _program,TemperatureSensorData _temperatureSensorData, File _resultsFile)throws Exception{
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			formatter.format("%1$-30s%n", "Program Name: " + program.getTTCSetUpDescriptionData().getProgramName());
		    formatter.flush ();
		    formatter.format("%1$-30s%n", "Program Author: " + program.getTTCSetUpDescriptionData().getProgramAuthor());
		    formatter.flush ();
		    formatter.format("%1$-30s%n", "Program Description: " + program.getTTCSetUpDescriptionData().getProgramDescription());
		    formatter.flush ();

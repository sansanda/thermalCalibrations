...
public class TTCProgramViewerFrame extends JFrame implements ActionListener
{
	...
	private static final int MIN_TEMP_OVEN = 0;
	 ...
	 TextField maxProfileTempTextField;
	 ...
	 public TTCProgramViewerFrame(ThermalCalibrationProgramData _program,int _mode){
		 ...
		 content_pane.setPreferredSize(frameDimension);
		 content_pane.setSize(frameDimension);
	 ...
	 private void createFrameComponents(){
		    ...
		    temperatureProfileList.addItemListener(new ItemListener()
		    {
		        public void itemStateChanged(ItemEvent e)
		        {
		        	...
		        	{
		        		oldValue = Integer.valueOf(numberOfTempProfileStepsChoice.getSelectedItem());
		        		newValue = oldValue + 1;
		        	...
		        	}else
		        	{
		        		...
		        		maxProfileTempTextField.setText(selectedItems[selectedItems.length-1]);
			...
			maxProfileTempTextField = new TextField();
			maxProfileTempTextField.enable(false);
			...
			stepsValueChoice.addItemListener(new ItemListener()
		    {
		        public void itemStateChanged(ItemEvent e)
		        {
		        	if(stepsValueChoice.getSelectedItem().equals("0")){
						setDefaultTempProfileData();
					}
			...
			numberOfTempProfileStepsChoice.addItemListener(new ItemListener()
		    {
		        public void itemStateChanged(ItemEvent e)
		        {
		        	switch(Integer.parseInt(stepsValueChoice.getSelectedItem())){
		        		case(0):
	 ...
	 private void setComponentsInFrame(Container _content_pane){
			...
			//4,8
			setComponentInGrid(_content_pane,maxProfileTempTextField,4,8,1,1,1,1,GridBagConstraints.WEST,GridBagConstraints.NONE);
			//4,7
	 ...
	 private void setDefaultTempProfileData(){
		 ...
		 maxProfileTempTextField.setText("0");
		 numberOfTempProfileStepsChoice.select("1");
		 temperatureProfileList.select(0);
		 stepsValueChoice.select("0");
	 }
  ...
  public void loadProgramData(ThermalCalibrationProgramData _program){
	  ...
	  maxProfileTempTextField.setText(String.valueOf(_program.getTemperatureProfileData().getTemperatures()[_program.getTemperatureProfileData().getTemperatures().length-1]));
  ...
  public void actionPerformed (ActionEvent event) {
	  ...
	  if (cmd.equals("Mark_0To150ºC")){
		  maxProfileTempTextField.setText("150");
		  minProfileTempChoice.select("0");
	  ...
	  if (cmd.equals("Mark_0To175ºC")){
		  maxProfileTempTextField.setText("175");
		  minProfileTempChoice.select("0");
	  ...
	  if (cmd.equals("Mark_0To200ºC")){
		  maxProfileTempTextField.setText("200");
		  minProfileTempChoice.select("0");
  ...
  private void fillTemperatureProfileList(){
	  ...
	  int stepsValue = Integer.valueOf(stepsValueChoice.getSelectedItem());
	  ...
	  }else
	  {
		  ...
		  maxProfileTempTextField.setText(String.valueOf((temperatureProfileNSteps-1)*stepsValue + temperatureProfileMinTemp));

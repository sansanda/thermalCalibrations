...
public class RunTTCalibrationSetUpAction implements ActionInterface{
	...
	private float read2700ChannelsAndSaveDataToFile(TTCSetUpData _program,TemperatureSensorData _temperatureSensorData) throws Exception{
		...
		resultRow[0] = pt100RealT;
        for (int i=0;i<_program.getTTCsToCalibrateData().getNDevicesToCalibrate();i++){
            printActionMessageAndProgressScreenMessage("Reading the Device Number "+Integer.toString(i+1)+". \n");
            resultRow[i+1] = Float.toString(K2700.measureAverage4WireResistance(_program.getTTCsToCalibrateData().getDevicesToCalibrate()[i].getTTC_Channel(),avg));
        }
        resultRow[_program.getTTCsToCalibrateData().getNDevicesToCalibrate()+1] = pt1004WResistance;
        resultRow[_program.getTTCsToCalibrateData().getNDevicesToCalibrate()+2] = ovenDisplayTemp;
        resultRow[_program.getTTCsToCalibrateData().getNDevicesToCalibrate()+3] = Integer.toString(actualCalibrationStep);
        resultRow[_program.getTTCsToCalibrateData().getNDevicesToCalibrate()+4] = sdf2.format(currentTimeInMillis);
        resultRow[_program.getTTCsToCalibrateData().getNDevicesToCalibrate()+5] = sdf.format(currentTimeInMillis);
        resultRow[_program.getTTCsToCalibrateData().getNDevicesToCalibrate()+6] = Integer.toString(_program.getTemperatureProfileData().getTemperatures()[actualCalibrationStep]);
	...
	private int createResultsTableHeaderAndFormatIt(TTCSetUpData _program,TemperatureSensorData _temperatureSensorData){
        ...
        resultsTableHeader = new Object[_program.getTTCsToCalibrateData().getNDevicesToCalibrate()+7];

    	for (int i=1;i<=(_program.getTTCsToCalibrateData().getNDevicesToCalibrate()+7);i++){
        ...
        for (int i=0;i<(_program.getTTCsToCalibrateData().getNDevicesToCalibrate());i++){
            resultsTableHeader[i+1] = "Res_"+_program.getTTCsToCalibrateData().getDevicesToCalibrate()[i].getDeviceReference()+"_Channel="+_program.getTTCsToCalibrateData().getDevicesToCalibrate()[i].getTTC_Channel();
        }
        resultsTableHeader[_program.getTTCsToCalibrateData().getNDevicesToCalibrate()+1] = "R_PT100(Ohms)_Channel="+_temperatureSensorData.getChannel();
        resultsTableHeader[_program.getTTCsToCalibrateData().getNDevicesToCalibrate()+2] = "Oven_Temp(ºC)";
        resultsTableHeader[_program.getTTCsToCalibrateData().getNDevicesToCalibrate()+3] = "#_step";
        resultsTableHeader[_program.getTTCsToCalibrateData().getNDevicesToCalibrate()+4] = "Hour";
        resultsTableHeader[_program.getTTCsToCalibrateData().getNDevicesToCalibrate()+5] = "Date";
        resultsTableHeader[_program.getTTCsToCalibrateData().getNDevicesToCalibrate()+6] = "SP_Temp(ºC)";
	...
	private int initializeResistanceCalibrationSetUp()throws Exception{
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
    		...
    		if (calibrationSetUp.getTTCsToCalibrateData().getNDevicesToCalibrate()==0){
	...
	public void configureFileChooserForTXT(JFileChooser _fileChooser){
		_fileChooser.setFileHidingEnabled(false);
		_fileChooser.setFileFilter(new TXTFileFilter());
	}

...
import profiles.DiodesCalibrationSetUpData;
...
import actionsForDiodeCalibration.RunADiodesalibrationSetUp_Action.ProgressScreenRefreshTask;
import controller.ActionRequest;
import controller.ActionResult;
import fileUtilities.TXTFilter;
import fileUtilities.XMLFilter;

public class ThermalCalibrationSetUp_Runner {
	 ...
	 private static String 		DATE_FORMAT_NOW = "dd-MM-yyyy";
	    ...
	    public ThermalCalibrationSetUp_Runner()throws Exception{
	    ...
	    public boolean execute(ActionRequest _req) throws IOException, Exception
	    {
			...
			if (testResistancesAndTemperatureSensor(diodesCalibrationSetUp,temperatureSensorData)==-1) return false;
	    ...
	    private int testResistancesAndTemperatureSensor(DiodesCalibrationSetUpData diodesCalibrationSetUp,TemperatureSensorData _temperatureSensorData) throws Exception{
	        ...
	        runProgramSwingWorker = new SwingWorker() {
	            public Object construct() {
	                try {
	                    ...
	                    determineRoomTemperatureByTime(diodesCalibrationSetUp.getTemperatureStabilizationCriteriumData().getFirstTemperatureStepStabilitzationTimeInMinutes(), temperatureSensorData);
	    ...
	    private void waitForTemperatureStandardDeviationMatch(double _desiredStDev, int _desiredNWindowsUnderStDev, DiodesCalibrationSetUpData _calibrationSetUp,TemperatureSensorData _temperatureSensorData) throws Exception{
	    ...
	    private int createResultsTableHeaderAndFormatIt(DiodesCalibrationSetUpData _program,TemperatureSensorData _temperatureSensorData){
	        ...
	        resultsTableHeader[_program.getDiodesToCalibrateData().getNDevicesToMeasure()+2] = "Oven_Temp(ºC)";
	        ...
	        resultsTableHeader[_program.getDiodesToCalibrateData().getNDevicesToMeasure()+6] = "SP_Temp(ºC)";

...
import multimeters.Keithley2700_v2;
...
public class RunTTCalibrationSetUpAction implements Action{
	...
	private TTC_Calibration_MainController 		mainController;
	...
	ProgressScreenFrame 								progressScreenFrame = null;
	...
	public void setMain(MainController _main){
		mainController = (TTC_Calibration_MainController)_main;
	}
	...
	public boolean execute(ActionRequest _req) throws IOException, Exception
	{
		...
		if (initializeTimer(progressScreenRefreshTimer,calibrationSetUp, new ProgressScreenRefreshTask())==-1)return false;
	...
	public int createCalibrationProgressScreen(TTCSetUpData _program,TemperatureSensorData _temperatureSensorData){
	...
	private int testResistancesAndTemperatureSensor(TTCSetUpData _program,TemperatureSensorData _temperatureSensorData) throws Exception{
        ...
        for (int i=0;i<_program.getTTCsToCalibrateData().getNDevicesToMeasure();i++){
            ...
            resultRow[i+1] = Double.toString((K2700.measureAverage4WireResistance(_program.getTTCsToCalibrateData().getDevicesToMeasureData()[i].getDeviceChannel(),avg)).getValue());
        }
		...
		runProgramSwingWorker = new SwingWorker() {
			public Object construct() {
				try {
		    		...
		    		waitForTemperatureStabilizationTime(calibrationSetUp, temperatureSensorData);
					...
					/*case 0: //Temperature Stabilitzation by St Dev case
						while (!fiTempProfile)
						{
							...
							waitForTemperatureStandardDeviationMatch(calibrationSetUp,temperatureSensorData);
	...
	private void determineRoomTemperatureByTime(TTCSetUpData _ttcSetUpData, TemperatureSensorData _temperatureSensorData) throws Exception
	{
		...
		//Wait for room temperature stabilization by time
		waitForTemperatureStabilizationTime(_ttcSetUpData, temperatureSensorData);
		roomTemperature = (K2700.measurePT100Temperature(_temperatureSensorData.getChannel())).getValue();
	}
	...
	private void waitForRoomTemperatureStabilization(double _desiredStDev, int _desiredNWindowsUnderStDev,int _measurementWindow, long _samplingPeriodeInMilliseconds, TTCSetUpData _ttcSetUpData,TemperatureSensorData _temperatureSensorData) throws Exception{
		double 	actualStepStandardDeviation = 0;
	...
	private void waitForTemperatureStabilizationTime(TTCSetUpData calibrationSetUp,TemperatureSensorData _temperatureSensorData) throws Exception{
		int _temperatureStabilizationTime = calibrationSetUp.getTemperatureStabilizationCriteriumData().getTemperatureStabilitzationTime();
	...
	private void waitForTemperatureStandardDeviationMatch(TTCSetUpData _ttcSetUpData,TemperatureSensorData _temperatureSensorData) throws Exception{
		double 	measuredTemperature = 0;
		double 	actualStepStandardDeviation = 0;
		int 	actualNWindowsUnderStDev = 0;

		double _desiredStDev = _ttcSetUpData.getTemperatureStabilizationCriteriumData().getStDev();
		double _desiredNWindowsUnderStDev = _ttcSetUpData.getTemperatureStabilizationCriteriumData().getNumberOfWindowsUnderStDev();
		...
		printActionMessageAndProgressScreenMessage("Standard Deviation = "+actualStepStandardDeviation);
		measuredTemperature = (K2700.measurePT100Temperature(_temperatureSensorData.getChannel())).getValue();

		if (	actualStepStandardDeviation<=_desiredStDev &
				isErrorBetweenMeasuredTempAndDesiredTemperatureOutOfAdmissible(measuredTemperature,_ttcSetUpData.getTemperatureProfileData().getTemperatures()[actualCalibrationStep],_ttcSetUpData.getTemperatureStabilizationCriteriumData().getMaxAdminssibleTemperatureError()))
		{
			actualNWindowsUnderStDev++;
			printActionMessageAndProgressScreenMessage("Standard Deviation Below Desired.");
			printActionMessageAndProgressScreenMessage("Actualizing Passes Below StandardDeviation = "+actualNWindowsUnderStDev);
	...
	private double read2700ChannelsAndSaveDataToFile(TTCSetUpData _program,TemperatureSensorData _temperatureSensorData) throws Exception{
		double pt100RealT = 0;
		double pt1004WResistance = 0;
		long currentTimeInMillis = 0;
		double ovenDisplayTemp = 0;
	...
	private int createResultsTableHeaderAndFormatIt(TTCSetUpData _program,TemperatureSensorData _temperatureSensorData){
        ...
        resultsTableHeader[_program.getTTCsToCalibrateData().getNDevicesToMeasure()+2] = "Oven_Temp(ºC)";
        ...
        resultsTableHeader[_program.getTTCsToCalibrateData().getNDevicesToMeasure()+6] = "SP_Temp(ºC)";
	...
	private int createInstruments()throws Exception{
		...
		K2700 = new Keithley2700_v2(instrumentsData.getMultimeterData().getComPort());
	...
	private int initializeTimer(Timer _t, TTCSetUpData _program, TimerTask _task){
		long _timePeriod = _program.getTemperatureStabilizationCriteriumData().getSamplingPeriode();
	...
	private void insertTempPointsInGraph(long _SystemTimeInMilliseconds,OvenMeasuresCluster _ovenMeasuresCluster){
	...
	class ProgressScreenRefreshTask extends TimerTask{
		public void run() {
			try {
				...
				TemperatureMeasure desiredOvenTemp = new TemperatureMeasure(elapsedTimeInMillis,"desiredOvenTemp",calibrationSetUp.getTemperatureProfileData().getTemperatures()[actualCalibrationStep]);
				TemperatureMeasure ovenTemp = new TemperatureMeasure(elapsedTimeInMillis,"ovenTemp",(E2404.readTemperature()).getValue());
				TemperatureMeasure pt100OvenTemp = new TemperatureMeasure(elapsedTimeInMillis,"pt100OvenTemp",(K2700.measurePT100Temperature(temperatureSensorData.getChannel())).getValue());
				ResistanceMeasure  pt100OvenResistance = new ResistanceMeasure(elapsedTimeInMillis,"pt100OvenResistance",(K2700.measure4WireResistance(temperatureSensorData.getChannel())).getValue());
				double ovenTempErrorInPercent = ((desiredOvenTemp.getTemperatureInCelsius().getValue()-pt100OvenTemp.getTemperatureInCelsius().getValue())/desiredOvenTemp.getTemperatureInCelsius().getValue())*100.0;
				double ovenOutputPowerInPercent = E2404.readOutputPower();
				...
				insertTempPointsInGraph(currentTimeInMillis,ovenMeasuresCluster);

...
public class StartCalibrationProgramAction implements Action{
	...
	public boolean execute(ActionRequest _req) throws IOException, Exception
	{
		...
		if (mainScreenFrame.createProgressScreen(program)==-1)return false;
	...
	private void runProgram(){
		worker = new SwingWorker() {
			public Object construct() {
				try {
					float pt100RealT = 0;
					mainScreenFrame.configureMainScreenMenuBarMode(RUN_PROGRAM_MODE);
	...
	private int goToTheFirstTemperatureSetPointAboveTheRealTemp(float _pt100RealT,ThermalCalibrationProgramData _program)throws Throwable{
		boolean trobat = _pt100RealT<_program.getTemperatureProfileData().getTemperatures()[actualProgramStep];

...
public class TTCalibrationProgressScreenFrame extends JFrame{
	 ...
	 */
	public TTCalibrationProgressScreenFrame(MainController _mainController) {
		super();
    ...
    public int initializeResultsTable(TTCSetUpData _program,TemperatureSensorData _temperatureSensorData){
    	if (defaultTableModel==null) defaultTableModel = new DefaultTableModel();
		defaultTableModel.setRowCount(0);
		defaultTableModel.setColumnCount(_program.getTTCsToCalibrateData().getNDevicesToMeasure()+6);
		...
		_data[5]="Real T(Ohms) "+"(MChannel="+_temperatureSensorData.getChannel()+")";
		 for (int i=6,j=0;i<(_program.getTTCsToCalibrateData().getNDevicesToMeasure()+6);i++,j++){
			 _data[i] = _program.getTTCsToCalibrateData().getDevicesToMeasureData()[j].getDeviceReference()+"(MChannel="+_program.getTTCsToCalibrateData().getDevicesToMeasureData()[j].getDeviceChannel()+")";
		 }

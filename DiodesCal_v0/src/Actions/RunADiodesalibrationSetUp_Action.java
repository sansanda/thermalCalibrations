...
public class RunADiodesalibrationSetUp_Action implements Action{
	...
	public boolean execute(ActionRequest _req) throws IOException, Exception
	{
		...
		if (initializeResultsFile(diodesCalibrationSetUp,temperatureSensorData,resultsFile)==-1) return false;
	...
	public int createCalibrationProgressScreen(DiodesCalibrationSetUp_Data _diodesCalibrationSetUp,TemperatureSensor_Data _temperatureSensorData){
		...
		progressScreenFrame.initializeResultsTable(_diodesCalibrationSetUp,_temperatureSensorData);
		return 0;
	}
				...
				try {
					...
					case 0: //Temperature Stabilitzation by St Dev case
						while (!fiTempProfile)
						{
							...
							printActionMessageAndProgressScreenMessage("Setting the TSP1 to "+diodesCalibrationSetUp.getTemperatureProfileData().getTemperatures()[actualCalibrationStep]+" ºC \n");
	...
	private float read2700ChannelsAndSaveDataToFile(DiodesCalibrationSetUp_Data _diodesCalibrationSetUp_Data,TemperatureSensor_Data _temperatureSensorData) throws Exception{
		...
		float[] channelsToMeasure= new float[_diodesCalibrationSetUp_Data.getDiodesToCalibrateData().getNDevicesToMeasure()];
		...
		for (int i=0;i<_diodesCalibrationSetUp_Data.getDiodesToCalibrateData().getNDevicesToMeasure();i++){
			...
			channelsToMeasure[i] = K2700.measureAverageVoltage(diodesCalibrationSetUp.getDiodesToCalibrateData().getDiodesToMeasureData()[i].getDeviceChannel(),avg);
		}
		...
		saveDataToFile(
				...
				Integer.toString(diodesCalibrationSetUp.getTemperatureProfileData().getTemperatures()[actualCalibrationStep]),
				ovenDisplayTemp,
				pt100RealT,
				pt1004WResistance,
				channelsToMeasure
	...
	private int initializeResultsFile(DiodesCalibrationSetUp_Data _program,TemperatureSensor_Data _temperatureSensorData, File _resultsFile)throws Exception{
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			formatter.format("%1$-30s%n", "Program Name: " + diodesCalibrationSetUp.getDiodesCalibrationSetUpDescriptionData().getProgramName());
		    formatter.flush ();
		    formatter.format("%1$-30s%n", "Program Author: " + diodesCalibrationSetUp.getDiodesCalibrationSetUpDescriptionData().getProgramAuthor());
		    formatter.flush ();
		    formatter.format("%1$-30s%n", "Program Description: " + diodesCalibrationSetUp.getDiodesCalibrationSetUpDescriptionData().getProgramDescription());
		    formatter.flush ();
		    ...
		    for (int i=0;i<_program.getDiodesToCalibrateData().getNDevicesToMeasure();i++){
		    	formatter.format("%1$-40s",_program.getDiodesToCalibrateData().getDiodesToMeasureData()[i].getDeviceReference()+"(MultimeterChannel="+_program.getDiodesToCalibrateData().getDiodesToMeasureData()[i].getDeviceChannel()+")");
			    formatter.flush ();
		    }
		    formatter.format("%n");
		    formatter.flush ();
	...
	private void saveDataToFile(DiodesCalibrationSetUp_Data _program,String _programStep, String _currentTime, String _desiredTemp,int _ovenDisplayTemp, float _realTemp,float _realTempInOhms,float[] _measuredChannels){
	...
	private void actualizeResultsTable(DiodesCalibrationSetUp_Data _program,String _programStep, String _currentTimeInMillis, String _desiredTemp,int _ovenDisplayTemp, float _realTemp,float _realTempInOhms,float[] _measuredChannels){
